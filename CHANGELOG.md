...
- ファイル選択と文字起こし開始を分離し、用語ヒントとAI整形を確認してから送信する操作に変更
- 処理中のファイル変更・設定変更・ボタン連打を防ぎ、APIの二重送信を防止

## 2026-09-10

- Supabase に文字起こしの利用日時、音声時間、文字数、モデル、token 数、処理時間、推定 API 料金、成功・失敗を保存
- 音声と文字起こし本文は保存せず、元ファイル名も既定で保存しない
- Basic 認証付きの `/history`、履歴 API、CSV 出力を追加
- Supabase 保存失敗時も文字起こし結果を返すフォールバックを追加
